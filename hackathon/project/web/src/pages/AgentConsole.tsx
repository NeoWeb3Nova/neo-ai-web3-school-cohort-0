...
export default function AgentConsole() {
  ...
  const friendlyReason = (raw: string): string => {
    ...
    if (raw.includes('PERMISSION_DENIED')) return t('agent.permissionDenied');
    return raw;
  };
  ...
  const simulatePayment = () => {
    ...
    const runStep = (index: number) => {
      if (index >= steps.length) {
        ...
        let reason = t('agent.allChecksPassed');

        if (!isAmountOk) {
          finalStatus = 'DENIED';
          reason = t('agent.amountPositive');
        } else if (!isInWhitelist) {
          finalStatus = 'DENIED';
          reason = t('agent.vendorNotWhitelist');
        } else if (!isPerTxOk) {
          finalStatus = 'DENIED';
          reason = t('agent.perTxExceeded');
        } else if (!isBudgetOk) {
          finalStatus = 'DENIED';
          reason = t('agent.budgetExceeded');
        }
              ...
              {result.tx && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                  ...
                  <div className="p-2 rounded-im">
                    ...
                    <p className="text-text-primary mt-0.5">{result.tx.amount.toFixed(2)} {t('common.usdc')}</p>
                  </div>
                  {result.tx.tx_hash && (
