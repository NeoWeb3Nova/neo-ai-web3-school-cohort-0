...
export default function Layout() {
  ...
  const themeLabel = themeMode === 'auto' ? t('theme.auto') : themeMode === 'light' ? t('theme.light') : t('theme.dark');
  const ThemeIcon = themeMode === 'auto' ? Monitor : themeMode === 'light' ? Sun : Moon;

  const navItems = [
    { path: '/', label: t('nav.dashboard'), icon: LayoutDashboard },
    { path: '/cards', label: t('nav.cards'), icon: CreditCard },
    { path: '/agent', label: t('nav.agent'), icon: Bot },
    { path: '/attack', label: t('nav.attack'), icon: ShieldAlert },
        ...
        <div className="mx-auto h-full flex items-center justify-between" style={{ maxWidth: 'var(--width-page)', padding: '0 24px' }}>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span className="hidden sm:inline">{i18n.language === 'zh' ? t('lang.zh') : t('lang.en')}</span>
            </button>
